...
import {
  ...
  Database, Eye, FileText, Zap, Activity, Search, Globe, Package, BarChart2, Clock, Code2, Network, Layers, KeyRound,
} from "lucide-react";
import "./index.css";
...
function AdvancedStaticPanel({advanced}){
  if(!advanced||advanced.status!=="completed")return null;
  const metrics=advanced.metrics||{};
  const signing=advanced.signing||{};
  const nativeCount=metrics.native_library_count??0;
  const deps=metrics.dependency_count??0;
  const flows=metrics.candidate_flow_count??0;
  const exported=metrics.exported_component_count??0;
  const deepLinks=metrics.deep_link_count??0;
  const schemes=(signing.detected_schemes||[]).join(", ")||((signing.v1_present)?"v1 only":"not detected");
  const cards=[
    [Layers,"Attack Surface",`${exported} exported · ${deepLinks} deep links`],
    [KeyRound,"Signing",schemes],
    [Code2,"Native & SDKs",`${nativeCount} native · ${deps} SDKs`],
    [Network,"Flow Candidates",`${flows} bounded static candidates`],
  ];
  return <div className="panel"><div className="panel-head"><Code2 size={16}/>Advanced Static Intelligence<span className="panel-count">Phase 3</span></div><div className="features-grid" style={{marginTop:12}}>{cards.map(([Icon,title,desc])=><div key={title} className="feature-card"><div className="feature-icon"><Icon size={16}/></div><div><div className="feature-title">{title}</div><div className="feature-desc">{desc}</div></div></div>)}</div><div className="empty-state" style={{marginTop:10,textAlign:"left"}}>Static call paths and source/sink candidates are not runtime proof. Signing scheme detection is structural unless verified by a controlled apksigner worker.</div></div>;
}

function SBOMPanel({sbom}){
  const components=sbom?.components||[];
  if(!components.length)return null;
  return <div className="panel"><div className="panel-head"><Package size={16}/>Software Bill of Materials<span className="panel-count">{components.length} components</span></div><div className="findings-list">{components.slice(0,20).map((component,i)=><div className="finding-card" key={`${component.name}-${i}`}><div className="finding-main"><div className="finding-title">{component.name}</div><div className="finding-desc">{component.purl||component.type} · version {component.version||"unknown"}</div></div></div>)}</div></div>;
}

function ResultsDashboard({data,onReset}){
  const findings=data.findings??data.ai_analysis?.findings??[];
  const score=(data.score?.final_score??data.risk_score??0);
  ...
  const advanced=data.advanced_static??null;
  ...
  const exportYaraRule = () => {
    ...
    meta:
        description = "Candidate exact-sample rule generated by APKGuard Industrial v3; validate before deployment"
        author = "APKGuard Industrial v3"
  ...
  return(
    <div className="dash-root">
      ...
      <AdvancedStaticPanel advanced={advanced}/><SBOMPanel sbom={data.sbom}/><ThreatIntelPanel intel={threatIntel}/><ScoreBreakdown breakdown={breakdown}/><MitreTable entries={mitre}/>
